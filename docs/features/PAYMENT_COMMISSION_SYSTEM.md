...
```json
{
  ...
  "amount": 60.0,
  "metadata": {
    "totalPaidBySeller": 100.0,
    "totalPaidToTesters": 40.0,
...
export default registerAs('stripe', () => ({
  ...
  testerTransferFee: parseFloat(
    process.env.TESTER_TRANSFER_FEE || process.env.PLATFORM_FEE || '10',
  ),
}));
```
...
if (campaign.sessions.length > 0) {
  throw new BadRequestException('Cannot refund campaign with active sessions');
}
```
