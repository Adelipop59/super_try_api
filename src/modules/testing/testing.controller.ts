...
export class TestingController {
  ...
  @Post('procedures/create-sample')
  @ApiOperation({
    summary: '[TEST] Créer une procédure de test avec données fictives',
    description:
      'Crée une procédure de test complète avec des données réalistes pour tester le système. Nécessite un campaignId existant.',
  })
  @ApiResponse({
    status: 201,
    description: 'Procédure de test créée avec succès',
  })
  async createSampleProcedure(@Body() body: { campaignId: string }) {
    const procedure = await this.prismaService.procedure.create({
      data: {
        campaignId: body.campaignId,
        title: 'Test Procédure - Déballage du produit',
        description:
          'Cette procédure de test guide le testeur dans le processus de déballage et vérification initiale du produit.',
        order: 1,
        isRequired: true,
      },
    });
    ...
    await this.logsService.logSuccess(
      LogCategory.TEST,
      `✅ [TEST] Procédure de test créée: ${procedure.title}`,
      { procedureId: procedure.id, campaignId: body.campaignId },
    );

    return {
      success: true,
      message: 'Procédure de test créée avec succès',
      data: procedure,
    };
  }
  ...
  @ApiOperation({
    summary: '[TEST] Lister toutes les procédures de test',
    description:
      'Retourne toutes les procédures existantes dans la base de données sans authentification.',
  })
  ...
  async listAllProcedures() {
    ...
    await this.logsService.logInfo(
      LogCategory.TEST,
      `🔵 [TEST] ${procedures.length} procédure(s) listée(s)`,
  ...
  @Delete('procedures/cleanup')
  @ApiOperation({
    summary: '[TEST] Supprimer toutes les procédures de test',
    description:
      'ATTENTION: Supprime TOUTES les procédures de la base de données. À utiliser uniquement en développement.',
  })
  @ApiResponse({ status: 200, description: 'Procédures supprimées' })
  async cleanupProcedures() {
    const count = await this.prismaService.procedure.count();
    await this.prismaService.procedure.deleteMany({});

    await this.logsService.logWarning(
      LogCategory.TEST,
      `⚠️ [TEST] ${count} procédure(s) supprimée(s)`,
      { count },
    );

    return {
      success: true,
      message: `${count} procédure(s) supprimée(s) avec succès`,
      count,
    };
  }
  ...
  @Post('steps/create-sample')
  @ApiOperation({
    summary: '[TEST] Créer des étapes de test avec données fictives',
    description:
      'Crée un ensemble complet d\'étapes de test (TEXT, PHOTO, VIDEO, CHECKLIST) pour une procédure.',
  })
  @ApiResponse({ status: 201, description: 'Étapes de test créées' })
  async createSampleSteps(@Body() body: { procedureId: string }) {
    const steps = await Promise.all([
      this.prismaService.step.create({
        data: {
          procedureId: body.procedureId,
          title: 'Ouvrir l\'emballage',
          description: 'Ouvrez délicatement l\'emballage du produit',
          type: StepType.TEXT,
          order: 1,
          isRequired: true,
        },
      ...
      this.prismaService.step.create({
        data: {
          procedureId: body.procedureId,
          title: 'Photo de l\'emballage',
          description: 'Prenez une photo de l\'emballage avant ouverture',
          type: StepType.PHOTO,
          order: 2,
          isRequired: true,
        },
      ...
      this.prismaService.step.create({
        data: {
          ...
          description: 'Vérifiez que tous les éléments sont présents',
          type: StepType.CHECKLIST,
          order: 3,
          isRequired: true,
          checklistItems: [
            'Produit principal présent',
      ...
      this.prismaService.step.create({
        data: {
          ...
          description: 'Filmez le processus de déballage complet',
          type: StepType.VIDEO,
          order: 4,
          isRequired: false,
        },
    ...
    await this.logsService.logSuccess(
      LogCategory.TEST,
      `✅ [TEST] ${steps.length} étape(s) de test créée(s)`,
      { procedureId: body.procedureId, count: steps.length },
    );

    return {
      success: true,
      message: `${steps.length} étape(s) de test créée(s) avec succès`,
      data: steps,
    };
  }
  ...
  @Get('steps/list-all')
  @ApiOperation({
    summary: '[TEST] Lister toutes les étapes de test',
    description:
      'Retourne toutes les étapes existantes dans la base de données.',
  })
  ...
  async listAllSteps() {
    ...
    await this.logsService.logInfo(
      LogCategory.TEST,
      `🔵 [TEST] ${steps.length} étape(s) listée(s)`,
      { count: steps.length },
    );

    return {
      success: true,
      message: `${steps.length} étape(s) trouvée(s)`,
      data: steps,
    };
  }

  @Delete('steps/cleanup')
  @ApiOperation({
    summary: '[TEST] Supprimer toutes les étapes de test',
    description:
      'ATTENTION: Supprime TOUTES les étapes de la base de données.',
  })
  @ApiResponse({ status: 200, description: 'Étapes supprimées' })
  async cleanupSteps() {
    const count = await this.prismaService.step.count();
    await this.prismaService.step.deleteMany({});

    await this.logsService.logWarning(
      LogCategory.TEST,
      `⚠️ [TEST] ${count} étape(s) supprimée(s)`,
      { count },
    );

    return {
      success: true,
      message: `${count} étape(s) supprimée(s) avec succès`,
      count,
    };
  }
