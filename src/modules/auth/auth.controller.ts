...
export class AuthController {
  ...
  @ApiResponse({ status: 200, description: 'Token rafraîchi avec succès', type: RefreshTokenResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'Email envoyé avec succès', type: MessageResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'Mot de passe réinitialisé avec succès', type: MessageResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'Email de vérification envoyé', type: MessageResponseDto })
  @ApiResponse({ status: 404, description: 'Utilisateur non trouvé' })
  async resendVerification(@Body() body: { email: string }): Promise<MessageResponseDto> {
    return this.authService.resendVerification(body.email);
  }

  @Public()
  @Get('health')
  @ApiOperation({ summary: 'Health check', description: 'Vérifie que le service d\'authentification fonctionne' })
  ...
  @ApiOperation({ summary: 'Vérifier token', description: 'Vérifie la validité du token JWT et retourne les infos utilisateur' })
  ...
  @ApiOperation({
    ...
    description: 'Change le mot de passe de l\'utilisateur connecté (nécessite l\'ancien mot de passe)'
  })
  @ApiResponse({ status: 200, description: 'Mot de passe modifié avec succès', type: MessageResponseDto })
  ...
  @ApiOperation({
    ...
    description: 'Change l\'adresse email de l\'utilisateur connecté (nécessite le mot de passe)'
  })
  @ApiResponse({ status: 200, description: 'Email mis à jour avec succès', type: MessageResponseDto })
  @ApiResponse({ status: 400, description: 'Mot de passe incorrect ou email déjà utilisé' })
  @ApiResponse({ status: 401, description: 'Token invalide ou manquant' })
  async updateEmail(
    @CurrentUser() user: AuthenticatedUser,
    @Body() updateEmailDto: UpdateEmailDto
  ): Promise<MessageResponseDto> {
    return this.authService.updateEmail(user.supabaseUserId, updateEmailDto.email, updateEmailDto.password);
  }
  ...
  @ApiOperation({
    ...
    description: 'Génère l\'URL de redirection pour l\'authentification OAuth (Google, GitHub, etc.)'
  })
  @ApiResponse({ status: 200, description: 'URL OAuth générée', type: OAuthUrlResponseDto })
  @ApiResponse({ status: 400, description: 'Provider non supporté' })
  async initiateOAuth(@Param('provider') provider: string): Promise<OAuthUrlResponseDto> {
  ...
  @ApiOperation({
    ...
    description: 'Gère le callback OAuth après authentification avec le provider externe'
  })
  @ApiResponse({ status: 200, description: 'Authentification OAuth réussie', type: AuthResponseDto })
